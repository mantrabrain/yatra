...
const DynamicPricingRuleForm: React.FC = () => {
  ...
  const { data: ruleData, isLoading, isError, error } = useQuery({
  ...
  // Log loading state
  console.log('Edit Mode:', isEdit, 'Rule ID:', ruleId, 'Loading:', isLoading, 'Error:', isError, error);
  console.log('ruleData:', ruleData);
  console.log('ruleData?.data:', ruleData?.data);
  console.log('ruleData?.data?.data:', ruleData?.data?.data);

  // Populate form with existing data
  useEffect(() => {
    console.log('useEffect triggered - ruleData:', ruleData);
    if (ruleData?.data) {
      const ruleDetails = ruleData.data;
      console.log('Populating form with:', ruleDetails);
      setFormData({
        ...
        weekend_days: Array.isArray(ruleDetails.weekend_days) ? ruleDetails.weekend_days : (ruleDetails.weekend_days ? JSON.parse(ruleDetails.weekend_days) : ['saturday', 'sunday']),
      });
    }
  }, [ruleData]);
    ...
    return (
      <div className="space-y-6">
        <PageHeader
          ...
          description={__('Configure dynamic pricing rules to automatically adjust trip prices')}
        />
        <Card>
          <CardContent className="pt-6">
            <div className="text-center py-12">
              ...
              <Button onClick={() => window.location.href = window.location.href.split('&action=')[0]}>
                {__('Back to Rules')}
              </Button>
            </div>
          </CardContent>
  ...
  const selectedRuleType = RULE_TYPES.find(rt => rt.id === formData.rule_type);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {!isEdit && !formData.rule_type && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>{__('Choose the type of dynamic pricing rule you want to create')}</CardDescription>
                </CardHeader>
                <CardContent>
                        ...
                        >
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <h3 className="font-semibold text-gray-900 dark:text-white mb-1">{ruleType.name}</h3>
                              <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">{ruleType.description}</p>
                              <p className="text-xs text-gray-500 dark:text-gray-500 italic">{__('Example:')} {ruleType.example}</p>
                            </div>
                          </div>
                        </button>
                      );
                ...
                <CardContent className="pt-6">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm text-gray-500 dark:text-gray-400">{__('Rule Type')}</p>
                        <p className="font-semibold text-gray-900 dark:text-white">{selectedRuleType.name}</p>
            ...
            {formData.rule_type && (
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>{__('Name and description for this pricing rule')}</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>{__('How much to adjust the price')}</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div>
                    <Label htmlFor="adjustment_type">{__('Adjustment Type')} *</Label>
                    <Select
                      id="adjustment_type"
                  ...
                  <div>
                    <Label htmlFor="adjustment_value">{__('Adjustment Value')} *</Label>
                    <Input
                      id="adjustment_value"
                      type="number"
                      step="0.01"
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                      {formData.adjustment_type === 'percentage' 
                        ? __('Enter percentage (e.g., 10 for 10% discount, -10 for 10% increase)')
                        : __('Enter fixed amount (e.g., 100 for $100 discount, -100 for $100 increase)')}
                    </p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>{__('When this rule should apply')}</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
            {/* Early Bird & Last Minute - Days before departure */}
            {(formData.rule_type === 'early_bird' || formData.rule_type === 'last_minute') && (
              <>
                <div>
                  <Label htmlFor="min_days_before">{__('Minimum Days Before Departure')}</Label>
                  <Input
                    id="min_days_before"
                    type="number"
                    value={formData.min_days_before}
                    onChange={(e) => handleChange('min_days_before', parseInt(e.target.value))}
                    min="0"
                  />
                </div>

                <div>
                  <Label htmlFor="max_days_before">{__('Maximum Days Before Departure')}</Label>
                  <Input
                    id="max_days_before"
                    type="number"
            ...
            {formData.rule_type === 'inventory' && (
              <>
                <div>
                  <Label htmlFor="min_inventory">{__('Minimum Available Seats')}</Label>
                  <Input
                    id="min_inventory"
                    type="number"
                    value={formData.min_inventory}
                    onChange={(e) => handleChange('min_inventory', parseInt(e.target.value))}
                    min="0"
                  />
                </div>

                <div>
                  <Label htmlFor="max_inventory">{__('Maximum Available Seats')}</Label>
                  <Input
                    id="max_inventory"
                    type="number"
                    value={formData.max_inventory}
                    onChange={(e) => handleChange('max_inventory', parseInt(e.target.value))}
                    min="0"
                  />
                </div>
              </>
            ...
            {formData.rule_type === 'demand' && (
              <>
                <div>
                  <Label htmlFor="demand_threshold_high">{__('High Demand Threshold (%)')}</Label>
                  <Input
                    id="demand_threshold_high"
                    type="number"
                    value={formData.demand_threshold_high}
                    onChange={(e) => handleChange('demand_threshold_high', parseInt(e.target.value))}
                    min="0"
                    max="100"
                  />
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                ...
                <div>
                  <Label htmlFor="demand_threshold_low">{__('Low Demand Threshold (%)')}</Label>
                  <Input
                    id="demand_threshold_low"
                    type="number"
                    value={formData.demand_threshold_low}
                    onChange={(e) => handleChange('demand_threshold_low', parseInt(e.target.value))}
                    min="0"
                    max="100"
                  />
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              ...
              <>
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) => handleChange('apply_on_weekends', e.target.checked)}
                      className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                    />
                    <Label htmlFor="apply_on_weekends" className="mb-0">{__('Weekends')}</Label>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) => handleChange('apply_on_weekdays', e.target.checked)}
                      className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                    />
                    <Label htmlFor="apply_on_weekdays" className="mb-0">{__('Weekdays')}</Label>
                  </div>
                </div>
                        ...
                        <div key={day} className="flex items-center gap-2">
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              if (e.target.checked) {
                                handleChange('weekend_days', [...current, day]);
                              } else {
                                handleChange('weekend_days', current.filter(d => d !== day));
                              }
                            }}
                            className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                          />
                          <Label htmlFor={`weekend_${day}`} className="mb-0 capitalize">{__(day)}</Label>
                        </div>
                      ))}
                    </div>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                      {__('Select which days are considered weekends for this rule')}
                    </p>
                  </div>
                )}
                ...
                <div className="p-3 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-lg">
                  <p className="text-xs text-indigo-700 dark:text-indigo-300">
                    {__('💡 Use positive values for premium pricing (weekends) or negative for discounts (weekdays).')}
                  </p>
                </div>
              </>
            )}
          ...
          {formData.rule_type && (
            <div className="space-y-4">
              ...
              <Card>
                <CardHeader className="pb-3">
                  <CardTitle className="text-base">{__('Applicable To')}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  <Select
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {formData.trip_ids.slice(0, 2).map((tripId) => {
                              ...
                              return (
                                ...
                                >
                                  {trip?.label || `Trip #${tripId}`} <span className="text-blue-600 dark:text-blue-400">#{tripId}</span>
                                  <button
                                    type="button"
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      handleChange('trip_ids', formData.trip_ids.filter(id => id !== tripId));
                                    }}
                        ...
                        <div className="absolute z-50 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg max-h-64 overflow-hidden">
                          ...
                          <div className="max-h-48 overflow-y-auto">
                            {tripOptions.map((trip: any) => {
                              const isSelected = formData.trip_ids.includes(trip.value);
                              return (
                                <div
                                  key={trip.value}
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    if (isSelected) {
                                      handleChange('trip_ids', formData.trip_ids.filter(id => id !== trip.value));
                                    } else {
                                      handleChange('trip_ids', [...formData.trip_ids, trip.value]);
                                    }
                                  }}
                                >
                                  ...
                                  <span className="flex-1 text-sm text-gray-900 dark:text-white">{trip.label}</span>
                                  <span className="text-xs text-gray-500 dark:text-gray-400">#{trip.value}</span>
                                </div>
                              );
                            })}
                          </div>
