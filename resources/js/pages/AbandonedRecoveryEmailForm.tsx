...
const AbandonedRecoveryEmailForm: React.FC = () => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['abandoned-recovery-campaigns'] });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={__('Configure automated email campaigns to recover abandoned bookings')}
      />
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                {__('Internal name for this campaign (not visible to customers)')}
              </p>
            </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => handleChange('delay_hours', parseInt(e.target.value))}
                min="1"
                max="168"
                required
              />
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                {__('How many hours after abandonment to send this email (1-168 hours)')}
              </p>
            </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>{__('Customize the email message')}</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              ...
              />
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                {__('Available variables: {customer_name}, {trip_name}, {booking_amount}')}
              </p>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder={__('Hi {customer_name},\n\nWe noticed you started booking {trip_name} but didn\'t complete it...')}
                required
                rows={10}
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-800 dark:text-white"
              />
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                {__('Available variables: {customer_name}, {trip_name}, {booking_amount}, {recovery_link}, {discount_code}')}
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>{__('Offer a discount to encourage completion')}</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-md border border-gray-200 dark:border-gray-700">
              <div className="mb-4">
                <strong className="text-sm text-gray-600 dark:text-gray-400">{__('Subject:')}</strong>
                <p className="text-gray-900 dark:text-white">{formData.subject || __('(No subject)')}</p>
              </div>
              <div>
                <strong className="text-sm text-gray-600 dark:text-gray-400">{__('Body:')}</strong>
              ...
              {formData.discount_code && (
                <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded">
                  <p className="text-sm text-blue-800 dark:text-blue-400">
                    <strong>{__('Discount:')}</strong> {formData.discount_code} - {formData.discount_amount}{formData.discount_type === 'percentage' ? '%' : ''} {__('off')}
                  </p>
                </div>
              )}
            </div>
