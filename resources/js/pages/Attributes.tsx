...
const Attributes: React.FC = () => {
  ...
  useEffect(() => {
    const handler = (event: MouseEvent) => {
      ...
      if (target.closest('[data-columns-trigger]') || target.closest('[data-columns-content]')) return;
  ...
  const apiErrorMessage = (attributesData as any)?.error || (attributesData as any)?.message;
  ...
  useEffect(() => {
    setSelectedIds((prev) => prev.filter((id) => attributes.some((a: Attribute) => a.id === id)));
  }, [attributes]);

  const attributeColumns = useMemo(() => {
    const formatBoolean = (value: boolean | string | number) => value === true || value === '1' || value === 1;
            ...
            <div className="w-10 h-10 rounded-lg bg-purple-100 dark:bg-purple-900/30 flex items-center justify-center flex-shrink-0">
              {attribute.icon ? (
                attribute.icon.type === 'image' ? (
                  <img src={attribute.icon.value} alt={attribute.name} className="w-6 h-6 object-cover rounded" />
                ) : (
                  <IconSelector iconName={attribute.icon.value} size={16} className="text-purple-600 dark:text-purple-400" />
                )
              ) : (
