...
const Availability: React.FC = () => {
  ...
  // Dates generated by the trip's recurring rules — shown READ-ONLY on the
  // calendar so a rules-only trip isn't a blank screen. Kept separate from the
  // main (specific-date) query so the paginated List view and its edit/delete/
  // bulk actions are untouched: those only ever operate on real rows.
  ...
  // Calendar-only dataset: the specific dates above PLUS read-only rule-generated
  // dates for any (date, time) that has no specific row. A specific row always
  // wins — the operator can edit it — so an existing manual override is never
  // masked by its rule. Only the calendar uses this; the List view keeps using
  // filteredDates untouched.
  const calendarDates = useMemo(() => {
    const rows = [...filteredDates];
    // Dedup against ALL specific rows, not just the filtered ones: a specific
    // override must mask its rule-generated twin even when the active status/
    // month filter has hidden that override — otherwise the calendar would show
    // the rule's "available" date over a real blocked/edited row.
    const seen = new Set(
      (availabilityData?.dates || []).map(
        (d: any) => `${d.departure_date}|${d.departure_time || ""}`,
      ),
    );
    const generated = (generatedData as any)?.dates || [];
    for (const g of generated) {
      const key = `${g.departure_date}|${g.departure_time || ""}`;
      ...
      rows.push({
        ...
        id: String(g.id),
        seats_remaining:
          (g.available_seats || 0) > 10
            ? "10+"
            : String(g.available_seats || 0),
        is_virtual: true,
      } as unknown as AvailabilityDate);
    }
    return rows;
                    ...
                    filteredDates.length === 0 ? (
                      ...
                      <div className="mt-4">
                        <AvailabilityCalendar
                          ...
                          onDateClick={(date) => {
                            // Rule-generated dates are read-only — they're edited
                            // on the Recurring Rules tab, not as individual rows.
                            if ((date as any).is_virtual) {
                              return;
                            }
                            navigate({
                              subpage: "trips",
