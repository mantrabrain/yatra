...
const DifficultyLevels: React.FC = () => {
  ...
  const actions = [
    ...
    },
    {
      ...
      onClick: (level: DifficultyLevel) =>
        statusMutation.mutate({ id: level.id, status: "publish" }),
    ...
    },
    {
      ...
      icon: <Edit className="w-4 h-4" />,
      onClick: (level: DifficultyLevel) =>
        statusMutation.mutate({ id: level.id, status: "draft" }),
      condition: (level: DifficultyLevel) =>
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: number) =>
      apiClient.delete(`/difficulty-levels/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["difficulty-levels"] });
      queryClient.invalidateQueries({ queryKey: ["difficulty-levels-stats"] });
      showToast(
        __("Difficulty level deleted successfully", "yatra"),
        "success",
      );
    },
    onError: (error: any) => {
      showToast(
        error?.message || __("Failed to delete difficulty level", "yatra"),
        "error",
      );
    },
  });
  ...
  const statusMutation = useMutation({
    ...
    onError: (error: any, variables) => {
      ...
      const fallback =
        ...
        __("Failed to update difficulty level", "yatra");
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span
        className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium ${info.className}`}
      >
        {info.label}
      </span>
    );
  ...
  const toggleColumn = (columnKey: string) => {
    ...
    localStorage.setItem(
      "yatra_difficulty_levels_columns",
    ...
    const performBulk = async () => {
      try {
        if (bulkAction === "delete") {
          ...
          await Promise.all(
            selectedIds.map(async (id) => {
              try {
                const levelResponse = await apiClient.get(
        ...
        const msgMapError: Record<string, string> = {
          ...
          trash: __(
            "Failed to move selected difficulty levels to trash",
            "yatra",
          ),
          draft: __(
            "Failed to mark selected difficulty levels as draft",
            "yatra",
          ),
          publish: __("Failed to publish selected difficulty levels", "yatra"),
        };
        showToast(error?.message || msgMapError[bulkAction], "error");
      }
    };
  ...
  return (
    <div className="space-y-3">
      ...
      <PageHeader
        ...
        description={__(
          "Manage trip difficulty levels and their ordering",
          "yatra",
        )}
        ...
        <CardContent className="p-0">
          <Table
            ...
            onSelectAll={(checked: boolean) => {
              if (checked) {
                setSelectedIds(
                  levels.map((level: DifficultyLevel) => level.id),
                );
              } else {
                setSelectedIds([]);
              }
