...
const Bookings: React.FC<BookingsProps> = ({
  ...
  if (searchTerm.trim()) {
    ...
    filteredDisplayBookings = filteredDisplayBookings.filter(
      (b: any) =>
        ...
        (b.booking_status &&
          b.booking_status.toLowerCase().includes(searchLower)),
    );
  }
  ...
  const bookingStats = {
    ...
    completed: displayBookings.filter((b) => b.booking_status === "completed")
      .length,
        ...
        <div className="flex-shrink-0 bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-100 dark:border-gray-700 min-w-0 flex-1">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xl font-bold text-gray-900 dark:text-white">
                {currency(bookingStats.totalSpent)}
              </p>
            </div>
      ...
      {filteredDisplayBookings.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-12 text-center">
          ...
          <p className="text-sm text-gray-400 dark:text-gray-500">
            {__("Try adjusting your filters or check back later.", "yatra")}
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {filteredDisplayBookings.map((booking) => {
            ...
            const canPayRemaining =
              amountDue > 0 && booking.payment_status !== "paid";
                  ...
                  <div className="yatra-booking-actions flex flex-wrap gap-3">
                    ...
                    {canPayRemaining && (
                      <button
                        type="button"
                        onClick={() =>
                          bookingId && startRemainingPaymentSession(bookingId)
                        }
