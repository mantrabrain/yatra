    ...
    <div className="yatra-documents-page space-y-6">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
              {__('Download itineraries, vouchers, invoices, and other documents for each trip.', 'yatra')}
            </p>
          </div>
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:gap-4">
            ...
            <div className="relative">
              <select
                ...
                onChange={(e) => setSelectedCategory(e.target.value as 'all' | 'itinerary' | 'voucher' | 'invoice' | 'downloads')}
      ...
      <div className="yatra-documents-stats flex flex-nowrap gap-6 overflow-x-auto">
        <div className="flex-shrink-0 yatra-document-stat-card bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-6 min-w-0 flex-1">
          <div>
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">{__('Itinerary', 'yatra')}</p>
            <p className="text-xl font-bold text-blue-600 dark:text-blue-400">{documentsByCategory.itinerary.length}</p>
          </div>
        </div>
        <div className="flex-shrink-0 yatra-document-stat-card bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-6 min-w-0 flex-1">
          <div>
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">{__('Voucher', 'yatra')}</p>
            <p className="text-xl font-bold text-emerald-600 dark:text-emerald-400">{documentsByCategory.voucher.length}</p>
          </div>
        </div>
        <div className="flex-shrink-0 yatra-document-stat-card bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-6 min-w-0 flex-1">
          <div>
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">{__('Invoice', 'yatra')}</p>
        ...
        <div className="flex-shrink-0 yatra-document-stat-card bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-6 min-w-0 flex-1">
          <div>
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">{__('Downloads', 'yatra')}</p>
            <p className="text-xl font-bold text-amber-600 dark:text-amber-400">{documentsByCategory.downloads.length}</p>
          </div>
        </div>
      </div>
      ...
      {filteredDocuments.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl p-12 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 font-medium mb-1">{__('No documents found', 'yatra')}</p>
          <p className="text-sm text-gray-400 dark:text-gray-500">{__('No documents are available yet. Please check back later.', 'yatra')}</p>
        </div>
      ) : (
                ...
                <div className="flex items-start gap-4">
                  <div className="flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {doc.category === 'downloads' && (
                          <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                            {__('Access', 'yatra')}: <span className="font-medium text-gray-700 dark:text-gray-300">{doc.access_label || doc.access || '-'}</span>
                            {doc.locked && doc.locked_reason ? (
                              <span className="ml-2 text-amber-700 dark:text-amber-300">({doc.locked_reason})</span>
                            ) : null}
                          </p>
                        )}
                      </div>
                    ...
                    <div className="yatra-document-actions flex flex-wrap gap-3 mt-4">
                      {doc.locked || !doc.url ? (
                        <div className="inline-flex items-center px-4 py-2 rounded-lg bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 transition-colors text-sm font-medium cursor-not-allowed" title={doc.locked_reason || __('Not available', 'yatra')}>
