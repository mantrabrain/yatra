        ...
        <div className="yatra-trip-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '24px' }}>
          {[1, 2, 3].map((i) => (
            <div key={i} className="yatra-trip-card">
              <div className="yatra-trip-image" style={{ backgroundColor: '#f3f4f6', minHeight: '200px' }}>
        ...
        <div className="yatra-trip-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '24px' }}>
          {savedTrips.map((trip: any) => {
            ...
            const displayPrice = trip.price !== undefined && trip.price !== null 
              ? trip.price 
              : (trip.sale_price !== undefined && trip.sale_price !== null && trip.sale_price > 0
                  ? trip.sale_price
                  : (trip.original_price !== undefined && trip.original_price !== null && trip.original_price > 0
                      ? trip.original_price
                      : 0));
            const originalPrice = trip.original_price;
            const avgRating = trip.rating !== undefined && trip.rating !== null ? trip.rating : (trip.average_rating !== undefined && trip.average_rating !== null ? trip.average_rating : 0);
            const reviewCount = trip.reviews !== undefined && trip.reviews !== null ? trip.reviews : (trip.review_count || trip.reviews_count || 0);
              ...
              >
                ...
                <div className="yatra-trip-content">
                  <div className="yatra-trip-meta">
                    {trip.location && (
                      <>
                        <span className="yatra-trip-location">{trip.location}</span>
                    ...
                    {trip.difficulty && (
                      <>
                        <span className="yatra-trip-separator">•</span>
                        <span className="yatra-trip-difficulty">{trip.difficulty}</span>
                      </>
                    )}
                  </div>
                  ...
                  {trip.highlights && trip.highlights.length > 0 && (
                    <div className="yatra-trip-highlights">
                      {trip.highlights.map((highlight: any, idx: number) => {
                        ...
                        if (highlightLink) {
                          return (
                            <a key={idx} href={highlightLink} className="yatra-highlight-badge yatra-highlight-link">
                              {highlightText}
                            </a>
                          );
                        }
                  ...
                  <div className="yatra-trip-footer">
                    <div className="yatra-trip-price">
                      {hasDiscount && originalPrice && originalPrice > displayPrice && !trip.is_traveler_based && (
                        <div className="yatra-original-price">{formatPrice(originalPrice)}</div>
                      )}
