...
const ConsentFormsList: React.FC = () => {
  ...
  useEffect(() => {
    setSelectedIds((prev) =>
      prev.filter((id) => forms.some((f: ConsentForm) => f.id === id)),
    );
  }, [forms]);
  ...
  return (
    <div className="space-y-3">
      ...
      <ConfirmationDialog
        ...
        onConfirm={() =>
          deleteConfirm.form && deleteMutation.mutate(deleteConfirm.form.id)
        }
        title={__("Delete Consent Form")}
        message={
          deleteConfirm.form
            ? __(
                'Are you sure you want to delete "{name}"? Forms with signed consents will be archived instead.',
              ).replace("{name}", deleteConfirm.form.name)
...
const SignedConsentsList: React.FC = () => {
  const [page, setPage] = useState(1);
  const [selectedConsent, setSelectedConsent] = useState<SignedConsent | null>(
    null,
  );
  ...
  const {
    ...
    isLoading,
    error,
  } = useQuery<SignedConsentsResponse>({
    queryKey: [
      "signed-consents",
      page,
      searchTerm,
  ...
  const handleDownloadPdf = async (consent: SignedConsent) => {
    try {
      ...
      if (data?.success && data?.data?.download_url) {
        ...
        showToast(
          __("Failed to prepare PDF download. Please try again.", "yatra"),
          "error",
        );
      }
    } catch (err: any) {
      ...
      <Card className="overflow-visible">
        <CardContent className="p-0 overflow-visible">
          <SharedTable
            ...
            columns={[
              ...
              },
              {
                ...
                render: (consent: SignedConsent) => (
                  <Badge
                    variant={
                      consent.status === "signed" ? "success" : "default"
                    }
                  >
            ...
            emptyDescription={__(
              "Signed consent forms will appear here once travelers submit them.",
            )}
        ...
        footer={
          selectedConsent && !detailError && !isDetailLoading ? (
            <div className="flex justify-end gap-3">
              ...
              <Button onClick={handleCloseModal}>{__("Close")}</Button>
            </div>
          ) : undefined
        }
      >
        {selectedConsent && (
          <>
            ...
            <div className="mb-6">
              ...
              <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 space-y-3">
                {selectedConsent.form_data &&
                  Object.entries(selectedConsent.form_data).map(
                    ([key, value]) => (
                      <div
                        key={key}
                        className="flex justify-between border-b border-gray-200 dark:border-gray-700 pb-2 last:border-0 last:pb-0"
                      >
                        <span className="text-sm text-gray-600 dark:text-gray-400">
                          {key}
                        </span>
                ...
                {(!selectedConsent.form_data ||
                  Object.keys(selectedConsent.form_data).length === 0) && (
                  <p className="text-sm text-gray-500">
                    {__("No form data recorded")}
                  </p>
                )}
              </div>
            </div>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={__(
          "Manage consent forms, waivers, and digital signatures for your trips.",
        )}
        actions={
          activeTab === "forms" ? (
            <Button
