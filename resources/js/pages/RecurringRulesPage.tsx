...
const RecurringRulesPage: React.FC = () => {
  ...
  const apiErrorMessage =
    (rulesResponse as any)?.error || (rulesResponse as any)?.message;
  ...
  const getWeekdaysLabel = (rule: RecurringRule): string => {
    const weekdays =
      ...
      (rule.days_of_week ? rule.days_of_week.split(",").map(Number) : []);
        ...
        render: (rule: RecurringRule) => (
          <div className="text-sm text-gray-700 dark:text-gray-300">
            {rule.rule_type === "weekly"
              ? getWeekdaysLabel(rule)
              : rule.rule_type === "interval"
                ? `Every ${rule.interval_days || 7} days`
                : rule.week_of_month
                  ? `${rule.week_of_month} week`
                  : "--"}
          </div>
        ),
      },
  ...
  return (
    <div className="space-y-6">
        ...
        message={
          deleteConfirm.rule
            ? __(
                ...
                "{name}",
                deleteConfirm.rule.name || `Rule #${deleteConfirm.rule.id}`,
              )
            : __(
      ...
      <PageHeader
        ...
        description={
          tripData
            ? `${__("Recurring availability rules for", "yatra")}: ${tripData.title}`
            : __("Manage recurring availability rules", "yatra")
        }
        actions={
              ...
              <SharedTable
                ...
                onRetry={() =>
                  queryClient.invalidateQueries({
                    queryKey: ["recurring-availability"],
                  })
                }
                ...
                onSelectItem={(id, checked) => {
                  if (checked) {
                    ...
                    setSelectedIds(
                      selectedIds.filter((sid) => sid !== id.toString()),
                    );
                  }
                }}
                onSelectAll={(checked) => {
                  if (checked) {
                    setSelectedIds(
                      rulesData.map((r: RecurringRule) => r.id.toString()),
                    );
                  } else {
                    setSelectedIds([]);
                  }
                ...
                isAllSelected={
                  selectedIds.length === rulesData.length &&
                  rulesData.length > 0
                }
