...
interface UseWordPressMediaReturn {
  openMediaLibrary: (callback: (attachment: MediaAttachment | MediaAttachment[]) => void) => void;
}

/**
 * Hook to interact with WordPress media library
 */
export const useWordPressMedia = (options: UseWordPressMediaOptions = {}): UseWordPressMediaReturn => {
  const {
  ...
  const openMediaLibrary = useCallback(
    (callback: (attachment: MediaAttachment | MediaAttachment[]) => void) => {
      ...
      const getWpMedia = (): any => {
        ...
        try {
          const wpMedia = (new Function('return (typeof wp !== "undefined" && typeof wp.media === "function") ? wp.media : null'))();
          if (wpMedia) {
            return wpMedia;
          }
        } catch (e) {
        ...
        try {
          if (typeof (window as any).wp === 'object' && (window as any).wp !== null && typeof (window as any).wp.media === 'function') {
        ...
        try {
          ...
          const wpMedia = eval('(typeof wp !== "undefined" && typeof wp.media === "function") ? wp.media : null');
          if (wpMedia) {
            return wpMedia;
          }
        } catch (e) {
      ...
      ensureMediaLibrary()
        .then((wpMedia) => {
          ...
          frame.on('select', () => {
            ...
            if (multiple) {
              const formattedAttachments: MediaAttachment[] = attachments.map((attachment: any) => ({
                id: attachment.id,
                url: attachment.url,
                alt: attachment.alt || '',
