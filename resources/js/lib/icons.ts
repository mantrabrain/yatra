...
export async function loadIcons(): Promise<Record<string, IconData>> {
  ...
  try {
    ...
    const response = await fetch('/wp-content/plugins/yatra/includes/icons.json');
    if (!response.ok) {
 ...
 */
export function getIconComponent(iconName: string): React.ComponentType<React.SVGProps<SVGSVGElement>> | null {
