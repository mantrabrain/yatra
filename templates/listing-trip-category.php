<?php
/**
 * Trip Category Listing Template
 *
 * Lists all published trip categories from the Yatra trip categories table
 * using the TripCategoryService. This replaces the previous static
...
// Load TripCategoryService from plugin namespace
if (!class_exists('Yatra\\Services\\TripCategoryService')) {
    // If the service is not available for some reason, bail gracefully.
    yatra_get_header();
    echo '<p>' . esc_html__('Trip category service is not available.', 'yatra') . '</p>';
    yatra_get_footer();
    return;
}
...
// Fetch published categories with stats (including trips_count, avg_rating, starting_price)
$categories = $category_service->getPublishedWithStats();
...
if (!empty($categories) && is_array($categories)) {
    yatra_sort_archive_listing_stats_rows($categories, $sort);
}
...
$paged_categories = $total_items > 0 ? array_slice($categories, $offset, $per_page) : [];
...
<style>
.yatra-category-icon-wrapper svg {
    width: 100% !important;
        ...
        <div class="yatra-listing-container">
            ...
            <div class="yatra-category-header">
                <div class="yatra-category-header-content">
                    <h1><?php esc_html_e('Trip Categories', 'yatra'); ?></h1>
                    <p class="yatra-archive-listing-lede"><?php esc_html_e('Browse trips by style or theme—adventure, family, luxury, and more. Open a category to find matching trips you can book.', 'yatra'); ?></p>
            ...
            <div class="yatra-category-grid" id="category-grid">
                <?php if (!empty($paged_categories)) : ?>
                    <?php foreach ($paged_categories as $category) : ?>
                        <?php
                        // Category is a stdClass from repository
                        ...
                        if (!empty($category->icon)) {
                            $icon = maybe_unserialize($category->icon);
                        ...
                        $has_avg_rating = isset($category->avg_rating) && (float) $category->avg_rating > 0;
                        $avg_rating     = $has_avg_rating ? (float) $category->avg_rating : 0.0;

                        $has_trips_count = isset($category->trips_count) && (int) $category->trips_count > 0;
                        $trips_count     = $has_trips_count ? (int) $category->trips_count : 0;

                        $starting_price_raw = isset($category->starting_price)
                            ? (float) $category->starting_price
                            : 0.0;
                        ...
                        // Build permalink
                        $permalink = function_exists('yatra_get_category_permalink')
                            ? yatra_get_category_permalink($category->id ?? $category)
                            : '';
                        ?>
                        <div class="yatra-category-card">
                            <div class="yatra-category-image">
                                ...
                                <?php elseif (!empty($icon_class)) : ?>
                                    <div class="yatra-category-icon-wrapper" aria-hidden="true" style="display: flex; align-items: center; justify-content: center; width: 100%; height: 100%; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white;">
                                        <?php echo yatra_svg_icon($icon_class, 'yatra-category-icon'); ?>
                                    </div>
                                <?php else : ?>
                                    <?php
                            ...
                            <div class="yatra-category-content">
                                ...
                                <?php if (!empty($permalink)) : ?>
                                    <a class="yatra-btn yatra-btn-primary yatra-archive-card-cta" href="<?php echo esc_url($permalink); ?>">
                                        <?php echo yatra_archive_listing_cta_icon_markup($icon_class, 'palette'); ?>
                                        <span><?php echo esc_html__('Explore Category', 'yatra'); ?></span>
                                    </a>
                                <?php else : ?>
                                    <button type="button" class="yatra-btn yatra-btn-primary yatra-archive-card-cta" disabled>
                                        <?php echo yatra_archive_listing_cta_icon_markup($icon_class, 'palette'); ?>
                                        <span><?php echo esc_html__('Explore Category', 'yatra'); ?></span>
                                    </button>
                                <?php endif; ?>
                    ...
                    <?php endforeach; ?>
                <?php else : ?>
                    <p class="yatra-no-categories">
                        <?php echo esc_html__('No categories found. Please add categories from the Yatra admin panel.', 'yatra'); ?>
                    </p>
                <?php endif; ?>
            </div>
