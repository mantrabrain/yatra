...
class TripRepository extends BaseRepository
{
    ...
    /**
     * Filter trips by attributes
     *
     * @param array $filters Existing filters
     * @param array $attributeFilters Attribute filters in format [attribute_id => value]
     * @return array Updated filters with attribute joins
     */
    public function filterByAttributes(array $filters = [], array $attributeFilters = []): array
    {
        ...
        // Using hardcoded table name since there's no dedicated repository for this table
        $tripAttributesTable = $wpdb->prefix . 'yatra_trip_attributes';
        // Using hardcoded table name since there's no dedicated repository for this table
        $attributesTable = $wpdb->prefix . 'yatra_attributes';
        
        // Check if trip attributes table exists
        $tableExists = $wpdb->get_var(
            $wpdb->prepare("SHOW TABLES LIKE %s", $tripAttributesTable)
        ) === $tripAttributesTable;
        ...
        // For each attribute filter, create a separate join to avoid conflicts
        $joinIndex = 0;
        foreach ($attributeFilters as $attributeId => $attributeValue) {
            $alias = 'ta_' . $joinIndex;
            
            $filters['joins'][] = [
                'type' => 'INNER JOIN',
                'table' => $tripAttributesTable . ' AS ' . $alias,
                'condition' => $this->table . '.id = ' . $alias . '.trip_id'
            ];
            
            // Add attribute condition
            $condition = $alias . '.attribute_id = %d';
            $params = [$attributeId];
            
            // Handle different attribute value types
            if (is_array($attributeValue)) {
                // For multi-select attributes
                $placeholders = implode(',', array_fill(0, count($attributeValue), '%s'));
                $condition .= ' AND ' . $alias . '.value IN (' . $placeholders . ')';
                $params = array_merge($params, $attributeValue);
            } else {
                // For single value attributes
                $condition .= ' AND ' . $alias . '.value = %s';
    ...
    /**
     * Decode included/excluded items JSON column stored    stored on itinerary entries
     */
