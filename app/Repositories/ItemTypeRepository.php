...
class ItemTypeRepository extends BaseRepository
{
    ...
    public function getStatusCounts(array $args = []): array
    {
        ...
        $type_condition = "WHERE type = 'item_type'";
    ...
    public function findBySlug(string $slug): ?\stdClass
    {
        ...
        $result = $this->wpdb->get_row(
            $this->wpdb->prepare(
                "SELECT * FROM `{$table}` WHERE type = 'item_type' AND slug = %s",
                $slug
            )
        );
    ...
    public function search(string $search, array $args = []): array
    {
        ...
        $search_where = $this->wpdb->prepare(
            "WHERE type = 'item_type' AND (name LIKE %s OR slug LIKE %s OR description LIKE %s)",
    ...
    public function countItemsByType(int $item_type_id): int
    {
        ...
        $count = $wpdb->get_var(
            $wpdb->prepare(
                "SELECT COUNT(*) FROM `{$table}` WHERE `type` = 'item' AND `parent_id` = %d",
                $item_type_id
            )
        );
        return (int) $count;
