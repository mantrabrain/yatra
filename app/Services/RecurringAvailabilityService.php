...
class RecurringAvailabilityService
{
    ...
    private function generateWeeklyDates(object $rule, string $fromDate, string $toDate): array
    {
        ...
        while ($current <= $end) {
            ...
            if (in_array($dayOfWeek, $targetDays, true)) {
                ...
                if (!$this->isDateExcluded($dateStr, $excludedDates)) {
                    // Check cutoff
        ...
        while ($current <= $end) {
            ...
            if ($targetDate) {
                ...
                if ($targetTimestamp >= strtotime($fromDate) && $targetTimestamp <= $end) {
                    // Check if not excluded
                    if (!$this->isDateExcluded($targetDate, $excludedDates)) {
                        // Check cutoff
     ...
     */
    /**
     * Has the operator explicitly closed this date on any of the trip's active
     * rules — a holiday, or a period such as a business vacation?
     *
     * Only answers for dates the operator deliberately listed. A date that is
     * simply not generated by a rule (a Tuesday on a Mon/Wed/Fri pattern) is
     * not "excluded" and is left to the rest of the resolution chain.
     */
    public function isDateExcludedForTrip(int $tripId, string $date): bool
    {
        if ($tripId < 1 || $date === '') {
            return false;
        }

        foreach ($this->repository->getActiveRulesForDateRange($tripId, $date, $date) as $rule) {
            if ($this->isDateExcluded($date, $rule->excluded_dates ?? [])) {
                return true;
            }
        }
    ...
    /**
     * Is this date excluded by the rule?
     *
     * An entry in `excluded_dates` is either a single 'Y-m-d' string (what the
     * admin has always been able to add one day at a time) or an inclusive
     * period ['start' => 'Y-m-d', 'end' => 'Y-m-d'] — a business vacation or
     * seasonal closure, added in one go instead of day by day.
     *
     * Both shapes share the one column so existing rules keep working
     * untouched: a stored list of plain strings behaves exactly as before.
     * Dates are ISO, so plain string comparison is chronological.
     *
     * @param string $date     Date being generated, 'Y-m-d'.
     * @param mixed  $excluded Stored exclusions (a list; anything else = none).
     */
    public function isDateExcluded(string $date, $excluded): bool
    {
        ...
        foreach ($excluded as $entry) {
            ...
            // Tolerate a reversed period rather than silently ignoring it.
            if ($start > $end) {
                [$start, $end] = [$end, $start];
            }
    ...
    public function previewDates(array $ruleData, int $limit = 20): array
    {
        ...
        // Normalise once: a malformed value (e.g. a bare string from a
        // hand-rolled API call) must not reach count() below as a fatal.
        $rule->excluded_dates = is_array($rule->excluded_dates ?? null)
            ? $rule->excluded_dates
            : [];
