...
class DiscountService extends BaseService
{
    ...
    /**
     * Get group discounts available for a specific trip
     * Used for frontend display of group discount availability
     */
    public function getGroupDiscountsForTrip(int $tripId): array
    {
        ...
        $all_discounts = $wpdb->get_results($query);
        
        // Filter discounts that apply to this trip
        $discounts = [];
        foreach ($all_discounts as $discount) {
            if ($discount->applicable_to === 'all') {
                $discounts[] = $discount;
            } elseif ($discount->applicable_to === 'specific_trips' && !empty($discount->trip_ids)) {
                // trip_ids is stored as PHP serialized array, use maybe_unserialize
                $trip_ids_array = maybe_unserialize($discount->trip_ids);
                if (is_array($trip_ids_array)) {
                    // Check if trip ID exists in array (handle both int and string values)
                    if (in_array($tripId, $trip_ids_array) || in_array((string)$tripId, $trip_ids_array)) {
                        $discounts[] = $discount;
                    }
                }
            }
        ...
        foreach ($discounts as $discount) {
            // Parse group_discount_ranges if available
            $ranges = [];
            if (!empty($discount->group_discount_ranges)) {
                $rangesData = json_decode($discount->group_discount_ranges, true);
                if (is_array($rangesData)) {
                    $ranges = $rangesData;
                }
            }

            // Parse category_discounts if available
            $categoryDiscounts = [];
            if (!empty($discount->category_discounts)) {
                $catData = json_decode($discount->category_discounts, true);
                if (is_array($catData)) {
                    $categoryDiscounts = $catData;
                }
            }
            ...
            $result[] = [
                'id' => (int) $discount->id,
                'code' => $discount->code,
                'min_group_size' => (int) $discount->min_group_size,
                'max_group_size' => $discount->max_group_size ? (int) $discount->max_group_size : null,
                'discount_type' => $discount->group_discount_type ?? 'percentage',
                'discount_amount' => (float) ($discount->group_discount_amount ?? 0),
                'discount_mode' => $discount->group_discount_mode ?? 'total',
                'group_discount_ranges' => $ranges,
                'category_discounts' => $categoryDiscounts,
                'range_label' => $this->formatGroupSizeRange($discount),
                'discount_label' => $this->formatDiscountAmount($discount),
            ];
        }
    ...
    /**
     * Calculate applicable group discount for a booking
     * 
