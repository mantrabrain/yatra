...
abstract class BaseService implements ServiceInterface
{
    ...
    protected function clearEntityCache(int $id): void
    {
        ...
        // The cache keys generated by getCacheKey() are prefixed with "service_{$entityType}_..."
        // and use a params hash, so prefix deletion by "{$entityType}_..." would miss them.
        // To avoid stale admin reads, clear the whole service keyspace and also delete the most
        // common single-entity keys directly.
        Cache::delete($this->getCacheKey('entity', ['id' => $id]));
        Cache::delete($this->getCacheKey('exists', ['id' => $id]));

        // Clear all caches for this service (covers all/all/count/paginate variants).
        $this->clearCacheByPattern("service_{$entityType}_");
    ...
    protected function clearAllServiceCache(): void
    {
        ...
        // Clear all caches related to this service (see getCacheKey()).
        $this->clearCacheByPattern("service_{$entityType}_");
