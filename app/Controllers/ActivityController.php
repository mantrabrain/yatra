...
class ActivityController extends BaseController
{
    ...
    public function register_routes(): void
    {
        ...
        // Register collection route (GET /activities, POST /activities)
        ...
        // Register single item route (GET /activities/{id}, PUT /activities/{id}, DELETE /activities/{id})
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('Yatra: Activity routes registration - Collection: ' . ($result1 ? 'SUCCESS' : 'FAILED'));
            error_log('Yatra: Activity routes registration - Single: ' . ($result2 ? 'SUCCESS' : 'FAILED'));
        }
    }
    ...
    public function check_permission(?WP_REST_Request $request = null): bool
    {
        ...
        switch ($method) {
            case 'GET':
                // View activities requires yatra_view_trips capability
                $has_permission = current_user_can('yatra_view_trips');
                break;
            
            case 'POST':
                // Create activity requires yatra_edit_trips capability
                $has_permission = current_user_can('yatra_edit_trips');
                break;
            
            case 'PUT':
            case 'PATCH':
                // Update activity requires yatra_edit_trips capability
                $has_permission = current_user_can('yatra_edit_trips');
                break;
            
            case 'DELETE':
                // Delete activity requires yatra_edit_trips capability
                $has_permission = current_user_can('yatra_edit_trips');
                break;
    ...
    public function get_item(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        try {
            ...
            if (!$item) {
                return $this->error_response('Activity not found', 404);
            }
    ...
    public function update_item(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        try {
            ...
            if (!$result) {
                return $this->error_response('Failed to update activity', 500);
            }
    ...
    public function delete_item(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        try {
            ...
            if (!$result) {
                return $this->error_response('Failed to delete activity', 500);
            }
