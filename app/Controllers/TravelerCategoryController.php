...
class TravelerCategoryController extends BaseController
{
    ...
    {
        $namespace = 'yatra/v1';
        $base = 'traveler-categories';

        // Register collection route (GET /traveler-categories, POST /traveler-categories)
        ...
        // Register single item route (GET /traveler-categories/{id}, PUT /traveler-categories/{id}, DELETE /traveler-categories/{id})
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('Yatra: TravelerCategory routes registration - Collection: ' . ($result1 ? 'SUCCESS' : 'FAILED'));
            error_log('Yatra: TravelerCategory routes registration - Single: ' . ($result2 ? 'SUCCESS' : 'FAILED'));
        }
    }

    /**
     * Check permissions for traveler category endpoints
     */
    public function check_permission(?WP_REST_Request $request = null): bool
    {
        ...
        switch ($method) {
            case 'GET':
                // View categories requires yatra_view_trips capability
                $has_permission = current_user_can('yatra_view_trips');
                break;
            
            case 'POST':
                // Create category requires yatra_edit_trips capability
                $has_permission = current_user_can('yatra_edit_trips');
                break;
            
            case 'PUT':
            case 'PATCH':
                // Update category requires yatra_edit_trips capability
                $has_permission = current_user_can('yatra_edit_trips');
                break;
            
            case 'DELETE':
                // Delete category requires yatra_edit_trips capability
                $has_permission = current_user_can('yatra_edit_trips');
                break;
    ...
    public function get_items(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        ...
        try {
            ...
            $allowed_order_by = ['id', 'label', 'slug', 'status', 'created_at', 'updated_at'];
    ...
    public function get_item(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        try {
            ...
            if (!$item) {
                return $this->error_response('Traveler category not found', 404);
            }
    ...
    public function create_item(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        try {
            ...
            return $this->success_response([
                'id' => $id,
                'message' => 'Traveler category created successfully',
            ], 201);
    ...
    public function update_item(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        try {
            ...
            if (!$result) {
                return $this->error_response('Failed to update traveler category', 500);
            }

            return $this->success_response([
                'message' => 'Traveler category updated successfully',
            ]);
        } catch (\InvalidArgumentException $e) {
            return $this->error_response($e->getMessage(), 400);
        } catch (\Exception $e) {
    ...
    public function delete_item(WP_REST_Request $request): WP_REST_Response|WP_Error
    {
        try {
            ...
            if (!$result) {
                return $this->error_response('Failed to delete traveler category', 500);
            }

            return $this->success_response([
                'message' => 'Traveler category deleted successfully',
            ]);
        } catch (\Exception $e) {
            return $this->error_response($e->getMessage(), 500);
        }
