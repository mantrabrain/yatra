 ...
 *
 * @class          Yatra_Log_Handler_DB
 */
class Yatra_Log_Handler_DB extends Yatra_Log_Handler
{
     ...
     * @see Yatra_Log_Handler_DB::get_log_source() for default source.
     *
     */
    ...
    protected static function add($timestamp, $level, $message, $source, $context)
    {
        ...
        $insert = array(
            ...
            'level' => Yatra_Log_Levels::get_level_severity($level),
            'message' => $message,
            'source' => $source,
        );
        ...
        return false !== $wpdb->insert("{$wpdb->prefix}yatra_logs", $insert, $format);
    }
    ...
    public static function flush()
    {
        ...
        return $wpdb->query("TRUNCATE TABLE {$wpdb->prefix}yatra_logs");
    }
    ...
    public function clear($source)
    {
        ...
        return $wpdb->query(
            $wpdb->prepare(
                "DELETE FROM {$wpdb->prefix}yatra_logs WHERE source = %s",
                $source
            )
        );
    }
    ...
    public static function delete($log_ids)
    {
        ...
        return $wpdb->query($wpdb->prepare("DELETE FROM {$wpdb->prefix}yatra_logs WHERE log_id IN {$query_in}", $log_ids)); // @codingStandardsIgnoreLine.
    }
    ...
    public static function delete_logs_before_timestamp($timestamp = 0)
    {
        ...
        $wpdb->query(
            $wpdb->prepare(
                "DELETE FROM {$wpdb->prefix}yatra_logs WHERE timestamp < %s",
    ...
    {
        static $ignore_files = array('class-yatra-log-handler-db', 'class-yatra-logger');
