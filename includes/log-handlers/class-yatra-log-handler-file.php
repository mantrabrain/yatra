 ...
 *
 * @class          Yatra_Log_Handler_File
 * @version        1.0.0
 */
class Yatra_Log_Handler_File extends Yatra_Log_Handler
{
    ...
    public function __construct($log_size_limit = null)
    {
        ...
        $this->log_size_limit = apply_filters('yatra_log_file_size_limit', $log_size_limit);
     ...
     *         originally used in deprecated Yatra_Logger::add calls.
     * }
     *
    ...
    public function clear($handle)
    {
        ...
        do_action('yatra_log_clear', $handle);
    ...
    public function remove($handle)
    {
        ...
        if (isset($logs[$handle]) && $logs[$handle]) {
            $file = realpath(trailingslashit(yatra()->get_log_dir()) . $logs[$handle]);
            if (0 === stripos($file, realpath(trailingslashit(yatra()->get_log_dir()))) && is_file($file) && is_writable($file)) { // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_is_writable
                $this->close($file); // Close first to be certain no processes keep it alive after it is unlinked.
                $removed = unlink($file); // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_unlink
            }
            do_action('yatra_log_remove', $handle, $removed);
        }
        return $removed;
    }
    ...
    public static function get_log_file_path($handle)
    {
        if (function_exists('wp_hash')) {
            return trailingslashit(yatra()->get_log_dir()) . self::get_log_file_name($handle);
        } else {
            _doing_it_wrong(__METHOD__, __('This method should not be called before plugins_loaded.', 'yatra'), '3.0');
            return false;
        }
    }
    ...
    public static function get_log_file_name($handle)
    {
        if (function_exists('wp_hash')) {
            ...
            _doing_it_wrong(__METHOD__, __('This method should not be called before plugins_loaded.', 'yatra'), '3.3');
            return false;
        }
    }
    ...
    public static function delete_logs_before_timestamp($timestamp = 0)
    {
        ...
        foreach ($log_files as $log_file) {
            $last_modified = filemtime(trailingslashit(yatra()->get_log_dir()) . $log_file);

            if ($last_modified < $timestamp) {
                @unlink(trailingslashit(yatra()->get_log_dir()) . $log_file); // @codingStandardsIgnoreLine.
            }
        }
    }
    ...
    {
        $files = @scandir(yatra()->get_log_dir()); // @codingStandardsIgnoreLine.
        $result = array();
