...
class Yatra_Block_Template_Utils
{
    /**
    ...
    public static function gutenberg_build_template_result_from_post($post)
    {
        ...
        if (!$terms) {
            return new \WP_Error('template_missing_theme', __('No theme is defined for this template.', 'yatra'));
        }
        ...
        // We are checking 'yatra' to maintain legacy templates which are saved to the DB,
        // prior to updating to use the correct slug.
        // More information found here: https://github.com/yatra/yatra-gutenberg-products-block/issues/5423.
        if (self::PLUGIN_SLUG === $theme || 'yatra' === strtolower($theme)) {
    ...
    public static function convert_slug_to_title($template_slug)
    {
        switch ($template_slug) {
            case 'single-product':
                return __('Single Product', 'yatra');
            case 'archive-product':
                return __('Product Archive', 'yatra');
            case 'taxonomy-product_cat':
                return __('Product Category', 'yatra');
            case 'taxonomy-product_tag':
                return __('Product Tag', 'yatra');
            default:
