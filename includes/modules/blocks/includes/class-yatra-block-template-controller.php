...
class Yatra_Block_Template_Controller
{
    ...
    public function __construct()
    {
        ...
        if (defined('YATRA_VERSION') && version_compare(YATRA_VERSION, '2.1.8', '>=')) {
            $root_path = YATRA_PLUGIN_DIR;
            $this->templates_directory = $root_path . Yatra_Block_Template_Utils::DIRECTORY_NAMES['TEMPLATES'];
            $this->template_parts_directory = $root_path . Yatra_Block_Template_Utils::DIRECTORY_NAMES['TEMPLATE_PARTS'];
            $this->init();
        }
    }
    ...
    public function maybe_return_blocks_template($template, $id, $template_type)
    {
        ...
        // Theme-based template didn't exist, try switching the theme to yatra and try again. This function has
        // been unhooked so won't run again.
        add_filter('get_block_file_template', array($this, 'get_single_block_template'), 10, 3);
        $maybe_template = function_exists('gutenberg_get_block_template') ?
            gutenberg_get_block_template(Yatra_Block_Template_Utils::PLUGIN_SLUG . '//' . $slug, $template_type) :
            get_block_template(Yatra_Block_Template_Utils::PLUGIN_SLUG . '//' . $slug, $template_type);
    ...
    public function get_single_block_template($template, $id, $template_type)
    {
        ...
        $available_templates = $this->get_block_templates_from_yatra(
            array($slug),
            $this->get_block_templates_from_db(array($slug), $template_type),
            $template_type
        );
        return (is_array($available_templates) && count($available_templates) > 0)
            ? Yatra_Block_Template_Utils::gutenberg_build_template_result_from_file($available_templates[0], $available_templates[0]->type)
            : $template;
    }
    ...
    {
        if (!Yatra_Block_Template_Utils::supports_block_templates()) {
            return $query_result;
        }
        ...
        foreach ($template_files as $template_file) {
            ...
            // adding duplicates.
            if (Yatra_Block_Template_Utils::set_has_theme_file_if_fallback_is_available($query_result, $template_file)) {
                continue;
            }
            ...
            if ('custom' !== $template_file->source) {
                $template = Yatra_Block_Template_Utils::gutenberg_build_template_result_from_file($template_file, $template_type);
            } else {
                $template_file->title = Yatra_Block_Template_Utils::convert_slug_to_title($template_file->slug);
    ...
    public function remove_theme_templates_with_custom_alternative($templates)
    {
        ...
        // for `yatra` in $template->source here because yatra templates won't have been added to $templates
    ...
    public function get_block_templates_from_db($slugs = array(), $template_type = 'wp_template')
    {
        ...
        // More context found here: https://github.com/yatra/yatra-gutenberg-products-block/issues/5423.
        $invalid_plugin_slug = 'yatra';
        ...
        $check_query_args = array(
            ...
            'tax_query' => array( // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_tax_query
                array(
                    'taxonomy' => 'wp_theme',
                    'field' => 'name',
                    'terms' => array($invalid_plugin_slug, Yatra_Block_Template_Utils::PLUGIN_SLUG, get_stylesheet()),
                ),
            ),
        );
        ...
        return array_map(
            function ($saved_woo_template) {
                return Yatra_Block_Template_Utils::gutenberg_build_template_result_from_post($saved_woo_template);
            },
            $saved_woo_templates
        );
    }
     ...
     */
    public function get_block_templates_from_yatra($slugs, $already_found_templates, $template_type = 'wp_template')
    {
        $directory = $this->get_templates_directory($template_type);
        $template_files = Yatra_Block_Template_Utils::gutenberg_get_template_paths($directory);

        $templates = array();

        foreach ($template_files as $template_file) {
            $template_slug = Yatra_Block_Template_Utils::generate_template_slug_from_path($template_file);
            ...
            if (
                Yatra_Block_Template_Utils::theme_has_template($template_slug) ||
                count(
                    array_filter(
            ...
            if (Yatra_Block_Template_Utils::template_is_eligible_for_product_archive_fallback($template_slug)) {
                $template_file = Yatra_Block_Template_Utils::get_theme_template_path('archive-product');
                $templates[] = Yatra_Block_Template_Utils::create_new_block_template_object($template_file, $template_type, $template_slug, true);
                continue;
            }

            // At this point the template only exists in the Blocks filesystem and has not been saved in the DB,
            // or superseded by the theme.
            $templates[] = Yatra_Block_Template_Utils::create_new_block_template_object($template_file, $template_type, $template_slug);
        }
    ...
    public function get_block_templates($slugs = array(), $template_type = 'wp_template')
    {
        ...
        $template_from_yatra = $this->get_block_templates_from_yatra($slugs, $templates_from_db, $template_type);
        $templates = array_merge($templates_from_db, $template_from_yatra);
        return Yatra_Block_Template_Utils::filter_block_templates_by_feature_flag($templates);
    ...
    {
        if (is_embed() || !Yatra_Block_Template_Utils::supports_block_templates()) {
            return;
        }

        if (
            is_singular('tour') &&
            !Yatra_Block_Template_Utils::theme_has_template('single-tour') &&
            $this->block_template_is_available('single-tour')
        ) {
            add_filter('yatra_has_block_template', '__return_true', 10, 0);
        }
    }
