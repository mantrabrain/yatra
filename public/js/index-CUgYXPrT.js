...
import { r as reactExports, j as jsxRuntimeExports, ap as X, ak as Info, ao as AlertTriangle, aC as AlertCircle, a_ as CheckCircle2 } from "./react-vendor-CnQxQzei.js";
...
const API_ENDPOINTS = {
  ...
  BOOKING_PAY_REMAINING: (id) => `/bookings/${id}/pay-remaining`,
  ...
  CUSTOMER_BOOKING_DOCUMENTS: (bookingId) => `/customer/bookings/${bookingId}/documents`,
  // Travelers
  TRAVELERS: "/travelers",
...
const apiService = {
  ...
  // Settings
  getSettings: (group) => apiClient.get(group ? API_ENDPOINTS.SETTINGS_GROUP(group) : API_ENDPOINTS.SETTINGS),
  // Payments
  getPayment: (id) => apiClient.get(API_ENDPOINTS.PAYMENT_GET(id)),
  ...
  bulkPaymentsAction: (action, ids) => Promise.all(ids.map((id) => {
    if (action === "delete") {
  ...
  performJobAction: (endpoint, jobId) => apiClient.post(API_ENDPOINTS.TOOLS_JOB_ACTION(endpoint, jobId)),
  ...
  bulkUpdateStatus: (endpoint, ids, status) => Promise.all(ids.map((id) => apiClient.put(`${endpoint}/${id}/status`, { status })))
};
const CURRENCIES = {
  // Major Currencies
...
//# sourceMappingURL=index-CUgYXPrT.js.map
